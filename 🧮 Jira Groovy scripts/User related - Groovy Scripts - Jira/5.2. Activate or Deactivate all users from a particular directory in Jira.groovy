...
//Created: 19th Mar 2023 - 10:37 am
//Last Modified:
////////////////////////////////////////
/*
Functionalities
...
//fetch all users from a directory
Set<String> userNameStringsarray = new HashSet<String>();
for(int u=0;u<allUsers.size();u++)
{
    ...
    {
        userNameStringsarray.add(allUsers[u].getUsername())
        count++
    }
}
printtext+="<br> Number of users: ${count}<br><br>"
 
//Choose to activate or deactivate the users
@Select(
...
for(int i=0;i<userNameStringsarray.size();)
{
  try
  {
    ...
    catch(Exception ex)
    {
      ...
      printtext=printtext+"${i+1}. <b>${userNameStringsarray[i]}</b> activation/deactivation not performed due to unknown errors.<br>"
    }
