/////////////////Rinaldi Michael///////////////////
//////Created in 27th January 2023, 08:59 am///////
/////Modified in 3rd April 2023, 09:17 pm//////////
///////////////////////////////////////////////////
...
@ShortTextInput(description = 'Enter a domain like gmail.com, yahoo.com ....etc. This will include users not added to the group but has the domain and is active. Number of groups input here should be the same as the number of groups set in the first field.', label = 'Enter the domain (Required*)')
String domainInput
String[] domains
if(domainInput!=null)
...
int notInGroup=0
printtext+="<h1><b>Active users from domain/s who are not part of the OpCo group-> ${domainInput}</h1></b><br><br>"
for(int j=0;j<allUsers.size();)
{
        String[] domain = allUsers[j].getEmailAddress().split('@')
        if(allUsers[j].getEmailAddress()=="")
            domain = allUsers[j].getUsername().split('@')
 
        //if(allUsers[j].getUsername().contains('mailto'))
        //    return allUsers[j].getEmailAddress()==null
 
        for(int d=0;d<domains.size();)
        {
            try
            {
            if (domain.contains(domains[d]) && userManager.getUserByName(allUsers[j].getName()).isActive()==true && groupManager.isUserInGroup(userManager.getUserByName(allUsers[j].getName()),groupone[d])==false)
            {
...
return "Number of users - ${userCount-1}. <br>${inGroup} users are assigned to the OpCo group. ${notInGroup} users are not in the OpCo group."+printtext
